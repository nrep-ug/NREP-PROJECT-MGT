...
/**
 * Verify if a user is a member of a project team.
 * If requiredRoles is provided, also checks that the user has at least one of those roles.
 * @param {string} userId - User account ID
 * @param {string} projectTeamId - Project team ID
 * @param {string[]} [requiredRoles] - Optional array of roles to require (e.g. ['manager', 'owner'])
 * @returns {Promise<boolean>}
 */
export async function verifyProjectMembership(userId, projectTeamId, requiredRoles) {
  try {
    if (!userId || !projectTeamId) {
      return false;
    }
    ...
    if (requiredRoles && requiredRoles.length > 0) {
      ...
      return memberships.memberships.some(
        m => m.userId === userId && m.roles.some(r => requiredRoles.includes(r))
      );
    }

    // Check if user is in the team (any role)
